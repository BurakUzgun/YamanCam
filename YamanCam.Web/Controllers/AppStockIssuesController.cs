...
/// <summary>
/// Stok Çıkış Fişi giriş ekranı. Seçilen şubeden stok çıkışı yapılır. Satırlar App_Stock
/// (malzeme kartları) referans alır, her satırda seçilen malzemenin birimi otomatik önerilir.
/// Fiş üzerinde döviz/KDV yoktur; yalnızca miktar x birim fiyat = toplam fiyat hesaplanır.
/// Yapı AppStockOpeningsController ile aynıdır.
/// </summary>
public class AppStockIssuesController : Controller
{
    ...
    public async Task<IActionResult> Index(bool showDeleted = false)
    {
        ...
        var query = _context.AppStockIssues.AsNoTracking();
        ...
        var items = await query
            ...
            .Select(x => new AppStockIssueListItemViewModel
            {
                RecId = x.RecId,
    ...
    public async Task<IActionResult> Create(AppStockIssueEditViewModel vm)
    {
        var denied = await EnsureAdmin();
        if (denied is not null)
        {
        ...
        var entity = new AppStockIssue { IsActive = true };
        ApplyViewModel(entity, vm);
        entity.CreatedDate = DateTime.UtcNow;
        ApplyLines(entity, vm);

        _context.AppStockIssues.Add(entity);
        await _context.SaveChangesAsync();

        await _appLogService.WriteInfoAsync(
            "StockIssue",
            "Created",
            $"Stok Çıkış Fişi oluşturuldu. No={entity.VoucherNo}",
            newValue: BuildAuditValue(entity));

        TempData["SuccessMessage"] = "Stok Çıkış Fişi kaydedildi.";
    ...
    public async Task<IActionResult> Edit(int id)
    {
        ...
        var entity = await _context.AppStockIssues
            .AsNoTracking()
            .Include(x => x.Lines)
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(int id, AppStockIssueEditViewModel vm)
    {
        var denied = await EnsureAdmin();
        if (denied is not null)
        {
        ...
        ApplyViewModel(entity, vm);
        _context.AppStockIssueLines.RemoveRange(entity.Lines);
        entity.Lines.Clear();
        ApplyLines(entity, vm);
        ...
        await _appLogService.WriteInfoAsync(
            "StockIssue",
            "Updated",
            $"Stok Çıkış Fişi güncellendi. No={entity.VoucherNo}",
            oldValue: oldAudit,
            newValue: BuildAuditValue(entity));

        TempData["SuccessMessage"] = "Stok Çıkış Fişi güncellendi.";
        return RedirectToAction(nameof(Index));
    }
    ...
    public async Task<IActionResult> Delete(int id)
    {
        ...
        await _appLogService.WriteInfoAsync(
            "StockIssue",
            "Deleted",
            $"Stok Çıkış Fişi silindi. No={entity.VoucherNo}",
            oldValue: oldAudit,
            newValue: BuildAuditValue(entity));

        TempData["SuccessMessage"] = "Stok Çıkış Fişi silindi.";
        return RedirectToAction(nameof(Index));
    }
    ...
    public async Task<IActionResult> Restore(int id)
    {
        ...
        await _appLogService.WriteInfoAsync(
            "StockIssue",
            "Restored",
            $"Stok Çıkış Fişi geri yüklendi. No={entity.VoucherNo}",
            oldValue: oldAudit,
            newValue: BuildAuditValue(entity));

        TempData["SuccessMessage"] = "Stok Çıkış Fişi geri yüklendi.";
    ...
    private async Task PopulateSelectListsAsync(AppStockIssueEditViewModel vm)
    {
        ...
        vm.TransactionTypeOptions = AppStockIssueTransactionTypes.All
    ...
    private AppStockIssueEditViewModel MapToViewModel(AppStockIssue entity)
    {
        return new AppStockIssueEditViewModel
        {
            RecId = entity.RecId,
            ...
            Lines = entity.Lines
                .OrderBy(x => x.LineNo)
                .Select(x => new AppStockIssueLineEditViewModel
                {
                    RecId = x.RecId,
                    LineNo = x.LineNo,
                    StockId = x.StockId,
    ...
    private static void ApplyViewModel(AppStockIssue entity, AppStockIssueEditViewModel vm)
    {
    ...
    private static void ApplyLines(AppStockIssue entity, AppStockIssueEditViewModel vm)
    {
        foreach (var line in vm.Lines)
        {
            entity.Lines.Add(new AppStockIssueLine
            {
                LineNo = line.LineNo,
    ...
    private static string BuildAuditValue(AppStockIssue entity)
    {
